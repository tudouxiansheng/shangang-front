<template>
  ...
  >
    ...
    <div
      ...
      class="scaleRect"
      :style="{
        '--x': scaleArea.leftTopX + 'px',
        '--y': scaleArea.leftTopY + 'px',
        '--w': scaleArea.rightBottomX - scaleArea.leftTopX + 'px',
        '--h': scaleArea.rightBottomY - scaleArea.leftTopY + 'px',
      }"
    ></div>

    <!-- 后台取点与前台展现点位些许差异为切图问题 -->
    <div class="cluster">
      <template v-if="fullPath === '/homepage'">
        <img
          v-for="(item, index) of clusterArr.filter((v) => v.child.length === 1)"
          :key="index + 100"
          :src="setIconImage(item.child[0])"
          :title="item.child[0].deviceName"
          class="single"
          :style="{
            '--x': setSingleIconPos(item.child[0], 'x') + 'px',
            '--y': setSingleIconPos(item.child[0], 'y') + 'px',
            '--scale': 1 / scale,
          }"
          @click="iconClick(item.child[0])"
        />
        <p
          v-for="(item, index) of clusterArr.filter((v) => v.child.length > 1)"
          :key="index + 1000"
          class="more"
          :style="{
            '--x': setMoreIconPos(item, 'x') + 'px',
            '--y': setMoreIconPos(item, 'y') + 'px',
            '--scale': 1 / scale,
          }"
        >
          {{ item.child.length }}
      ...
      <template v-if="fullPath === '/trail'">
        <img
          ...
          class="single"
          :style="{
            '--x': setSingleIconPos(item, 'x') + 'px',
            '--y': setSingleIconPos(item, 'y') + 'px',
            '--scale': 1 / scale,
          }"
...
export default {
    ...
    '$route.path': async function (newVal, oldVal) {
      ...
      if (newVal === '/homepage') {
        ...
        if (res.type === 'load') {
          ...
          this.cluster(this.scsDeviceInfoList)
        }
      }
      if (newVal === '/trail') {
        const res = await this.getBgImgWH()
  ...
  methods: {
    async getDeviceData() {
      ...
      this.scsDeviceInfoList = this.scsDeviceInfoList
        .concat(
        ...
        .map((v) => {
          v.x = this.getDevicePos(v, 'x')
          v.y = this.getDevicePos(v, 'y')
          return v
        })
    },
    getBgImgWH() {
    ...
    setSingleIconPos(val, type) {
      if (type === 'x') {
        return this.getDevicePos(val, type) - this.singleIconW / 2
      } else if (type === 'y') {
        return this.getDevicePos(val, type) - this.singleIconH
      }
    },
    setMoreIconPos(val, type) {
      if (type === 'x') {
        return val.centerX - this.moreIconW / 2
      } else if (type === 'y') {
        return val.centerY - this.moreIconH
      }
    },
    ...
    setDetailsPos(val) {
      this.curDevice = {
        ...
        top: this.getDevicePos(val, 'y') - this.popupH - this.singleIconH / this.scale,
      }
    },
    detailsPopupClose() {
    ...
    setAlarmPos(val) {
      this.alarmDevice = {
        ...
        top: this.getDevicePos(val, 'y') - this.alarmH - this.singleIconH / this.scale,
      }
    },
    /**
    ...
    drawLineWithArrow(type, data) {
      data = data.filter((v) => v.xeightyAxis && v.yeightyAxis)
      ...
      const arr2 = arr.filter((v) => !(v.fx === v.tx && v.fy === v.ty))
      if (arr2.legnth === 0) return
      ...
      arr2.forEach((v) => this.draw(ctx, v.fx, v.fy, v.tx, v.ty, 20, 20))
