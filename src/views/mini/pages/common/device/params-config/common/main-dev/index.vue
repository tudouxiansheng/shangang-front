  ...
  >
    ...
    <div :class="['center_container', cuType === '1' && 'full_height']">
      ...
      <el-tabs v-model="activeMenuName">
        <el-tab-pane :label="$t('mainDevConfiguration.systemParam')" name="system"></el-tab-pane>
        <el-tab-pane
          v-if="support && !mainDevIsExDomain"
          :label="$t('mainDevConfiguration.videoChannel')"
          name="video"
        ></el-tab-pane>
        <el-tab-pane
          v-if="mainDevStatus == 1 && drive != 'T28181' && support && !mainDevIsExDomain"
          :label="$t('mainDevConfiguration.audioChannel')"
          name="audio"
        ></el-tab-pane>
        <el-tab-pane
          v-if="
            ...
            !mainDevIsExDomain
          "
          :label="$t('mainDevConfiguration.serialChannel')"
          name="serial"
        ></el-tab-pane>
        <el-tab-pane
          v-if="mainDevStatus == 1 && drive != 'T28181' && support && !mainDevIsExDomain"
          :label="$t('mainDevConfiguration.alarmChannel')"
          name="alarm"
        ></el-tab-pane>
        <el-tab-pane
          v-if="mainDevStatus == 1 && platType == 3 && drive == 'HWSDK-ACREG' && !mainDevIsExDomain"
          :label="$t('mainDevConfiguration.intelligentAnalysis')"
          name="intelligent"
        ></el-tab-pane>
      </el-tabs>
      <div class="form_component">
        ...
        <intelligent-analysis v-if="activeMenuName === 'intelligent' && !pageLoading" />
      </div>
    </div>
    <el-dialog
...
import IntelligentAnalysis from './component/intelligent-analysis'
...
export default {
  ...
  computed: {
    ...
    support() {
      return !(this.platType == 3 && (this.drive == 'HIKSDK' || this.drive == 'DHSDK'))
    }
  },
  async mounted() {
    if (this.cuType === '1') {
