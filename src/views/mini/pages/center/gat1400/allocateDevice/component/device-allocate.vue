<template>
  <el-dialog
    :title="$t('cascadeDevice.distributionEquipment')"
    v-model="$parent.dialogvisible_allocate"
    top="10vh"
    width="1000px"
    class="deviceAllocateDialog"
  ...
  >
    <p style="color: #f56c6c">
      {{ $t('cascadeDevice.selectDistributionEquipment') }}
    </p>
    <div style="display: flex; justify-content: space-around">
      ...
      >
        ...
        >
          ...
          <el-table-column :label="$t('deviceManagement.deviceName')" show-overflow-tooltip>
            <template #default="{ row }">
              <span>
                <i class="aci-camera1 aci" style="margin-right: 5px"></i>
                {{ row.name }}
              </span>
            </template>
          </el-table-column>
    ...
    <batch-operate
      ...
      :what="$t('deviceManagement.device')"
      @getList="getList"
  ...
  methods: {
    ...
    async submitFormtree() {
      if (!this.multipleSelection.length) {
        this.$message({
          message: this.$t('cascadeDevice.selectDevice'),
          type: 'warning'
        })
        return
      }
      ...
      for (let i = 0; i < this.multipleSelection.length; i++) {
        this.multipleSelectionMany.push({
          ...
          thirdId: this.multipleSelection[i].apeID,
...
.deviceAllocateDialog .el-table .choose-row,
.deviceAllocateDialog .el-dialog__body .el-table__body tr.choose-row > td {
