<template>
  <el-dialog
    :title="$t('tenantManage.tenantDetail')"
    v-model="$parent.tenantDetailVisible"
    top="10vh"
    :width="$parent.locale == 'en' ? '1100px' : '1000px'"
    :close-on-click-modal="false"
    class="tenantDetailDialog detailDialog"
    :modal="false"
    draggable
    @closed="dialogClosed"
    @open="dialogOpen"
  >
    <el-form
      ref="form"
      :model="form"
      :label-width="$parent.locale == 'en' ? '225px' : '140px'"
      label-position="left"
    >
      <div class="form-flex">
        <div class="form-left">
          <el-form-item :label="$t('tenantManage.tenantName') + '：'">
          ...
          >
            <span>{{ setIndustryType(form.industryType) }}</span>
          </el-form-item>
        </div>
...
export default {
  ...
  methods: {
    ...
    dialogClosed() {
      this.$parent.tenantDetailVisible = false
    ...
    setIndustryType(val) {
      let name = ''
      this.$parent.industryTypeList.map(item => {
        if (val == item.value) {
          name = item.name
        }
      })
