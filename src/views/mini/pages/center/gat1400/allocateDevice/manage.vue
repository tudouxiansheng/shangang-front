<template>
  <div id="allocateDeviceManage" v-if="isRouterAlive" ref="allocateDeviceManage">
    <el-row v-if="!showSubInfo">
      ...
      <el-col :span="20" class="el-row20">
        <div style="margin: 20px 0 20px">
          <el-button :disabled="isFlag" type="primary" @click="handleSync" v-if="PermissionManage">
          ...
          >
            {{ $t('cascadeDevice.equipmentAllocation') }}
          </el-button>
        </div>
        ...
        <div class="search-form">
          <el-form
            ...
            :label-width="locale == 'en' ? '170px' : '84px'"
            ref="form"
            :model="searchForm"
            label-position="left"
          >
    ...
    <!-- //设备分配弹框 -->
    <device-allocate></device-allocate>
    <!-- //设备详情弹框 -->
...
import deviceAllocate from './component/device-allocate'
...
export default {
  ...
  components: {
    'device-allocate': deviceAllocate,
  ...
  methods: {
    ...
    getViewPlatList() {
      ...
      this.$api.selectVirApe(data).then(res => {
    ...
    // 字典查询
    getDictionary() {
      this.getSysDictData(['direction_type', 'vehicle_snapshot_direction'])
    },
