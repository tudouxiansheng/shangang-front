    ...
    >
      ...
      >
        <el-upload
          ...
          action="#"
          :limit="imageMaxNum"
          :file-list="initialUploadFiles"
          :on-exceed="handleExceed"
        >
          <el-icon class="image-uploader-icon"><el-icon-plus /></el-icon>
          <template #file="{ file }">
            <div>
...
import { repositoryType as RepositoryTypeEnum, genderList } from './constant'
import {
  PhoneRegx,
  NameRegx,
  LpnRegx,
...
export default {
  ...
  methods: {
    ...
    handleRemove(file) {
      // 从初始图片文件列表中移除被删除的文件
      const initialFileList = this.initialUploadFiles.filter(f => f.uid !== file.uid)
      this.initialUploadFiles = initialFileList
    },
    closeViewer() {
      this.viewerUrlList = []
    ...
    beforeUpload(file) {
      ...
      this.initialFileObjectUrl = URL.createObjectURL(file)
      let obj = {
        name: file.name,
        percentage: 0,
        raw: file,
      ...
      this.initialUploadFiles.push(obj)
    ...
    async submitForm() {
      ...
      const uploaderFiles = this.initialUploadFiles || []
      if (uploaderFiles.length === 0) {
        this.$message.warning(this.$t('repo.selectOneImg'))
        return false
      }
      this.loading = true
      try {
        ...
        if (this.isPersonRepo) {
          formData = {
            ...
            name: this.$loadash.isNull(this.form.name) ? undefined : this.form.name,
            bornTime: this.$loadash.isNull(this.form.bornTime) ? undefined : this.form.bornTime,
            country: this.form.country,
            credentialType: this.$loadash.isNull(this.form.credentialType)
              ? undefined
              : this.form.credentialType,
            credentialNumber: this.$loadash.isNull(this.form.credentialNumber)
              ? undefined
              : this.form.credentialNumber,
            gender: this.form.gender,
            nationality: this.form.nationality,
            occupation: this.form.occupation,
            age: this.form.age,
            phoneNum: this.$loadash.isNull(this.form.phoneNum) ? undefined : this.form.phoneNum,
          ...
          formData = {
            ...
            repositoryId: this.repositoryId,
            ownerName: this.$loadash.isNull(this.form.ownerName) ? undefined : this.form.ownerName,
            lpn: this.form.lpn,
        ...
        for (let i = 0; i < uploaderFiles.length; i++) {
          let file = uploaderFiles[i]
          let api
          if (this.objectId === '') {
            api = this.isPersonRepo
              ? '/api/ars/repository/person/add'
              : '/api/ars/repository/vehicle/add'
          } else {
            api = this.isPersonRepo
              ? '/api/ars/repository/person/modify'
              : '/api/ars/repository/vehicle/modify'
          }
          if (file.raw) {
            const imageFormData = new FormData()
            imageFormData.append('file', file.raw)
            imageFormData.append('api', api)
            imageFormData.append('netType', this.$getNetType())
            const res = await this.$api.uploadFileTemporary(imageFormData)
            if (res.fileId) {
              formData.patchPicture.push(res.fileId)
            } else {
              this.$message.error(this.$t('repo.uploadImgFail'))
              return false
            }
          } else if (file.tupleId) {
            formData[this.isPersonRepo ? 'faceList' : 'vehiclePhotos'].push({
              pictureId: file.name,
              tupleId: file.tupleId
            })
          }
        }
