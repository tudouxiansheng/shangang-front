<template>
  ...
  >
    <el-form
      ...
      label-position="left"
    >
      <collapse-card :title="$t('adminManage.adminInfo')" v-model:collapse="show1">
        <div class="form-flex">
          <div class="form-left">
            <el-form-item
        ...
        <el-form-item :label="$t('public.remark') + '：'" prop="remark" class="padding20">
          <el-input
            ...
            v-model="form.remark"
            auto-complete="off"
            :placeholder="$t('public.validate.charLimit', { num: 256 })"
            maxlength="256"
            show-word-limit
            clearable
          />
      ...
      <collapse-card :title="$t('adminManage.permissionSettings')" v-model:collapse="show2">
        <div class="form-flex">
          <div class="form-left">
            <!-- 会话控制数 -->
        ...
        <el-form-item
          :label="$t('adminManage.PTZControlPriority') + '：'"
...
export default {
  ...
  data() {
    ...
    var validateUsername = (rule, value, callback) => {
      if (value === '') {
        callback(new Error(this.$t('adminManage.namePrompt')))
      } else {
        if (!reg.test(value)) {
  ...
  methods: {
    ...
    getSliderMax() {
      ...
      for (let i = 1; i <= this.sliderMax; i++) {
        sliderMarks[i] =
          i == 1
            ? '1' + '(' + this.$t('adminManage.lowest') + ')'
            : i == this.sliderMax
            ? this.sliderMax + '(' + this.$t('adminManage.highest') + ')'
            : String(i)
      }
    ...
    submitForm() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          if (!this.$parent.operation) {
            Object.assign(data, { adminId: form.adminId })
            // 编辑管理员
            this.$api.modifyAdminInfo(data).then(res => {
              this.successResult(res, this.$t('public.modifySuccess'))
            })
          } else {
            // 添加管理员
            this.$api.addAdminInfo(data).then(res => {
