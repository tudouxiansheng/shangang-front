<template>
  <el-dialog
    :title="$parent.operation ? $t('userManage.addUser') : $t('userManage.editUser')"
  ...
  >
    <el-form
      ...
      label-position="left"
    >
      <collapse-card :title="$t('userManage.userInfo')" v-model:collapse="show1">
        <div class="form-flex">
          <div class="form-left">
            <el-form-item
          ...
          <div class="form-right">
            ...
            >
              <el-input
                ...
                type="password"
                auto-complete="off"
                :placeholder="$t('public.confirmPsdPrompt')"
                clearable
              />
            </el-form-item>
          </div>
      ...
      <collapse-card :title="$t('userManage.permissionSettings')" v-model:collapse="show2">
        <div class="form-flex">
          <div class="form-left">
            <!-- 会话控制数 -->
...
export default {
  ...
  data() {
    ...
    var validateUsername = (rule, value, callback) => {
      if (value === '') {
        callback(new Error(this.$t('userManage.namePrompt')))
      } else {
        if (!reg.test(value)) {
  ...
  async mounted() {
    this.getSliderMax()
    await this.getComplexity_rules() // 判断是否有角色权限
  },
  activated() {
  ...
  methods: {
      ...
      this.$nextTick(() => {
        if (this.isEnable) {
          this.rules.password = [
            ...
            {
              min: this.minLength || 8,
              max: 16,
              message: this.$t('public.psdPrompt1', { min: this.minLength || 8 }),
              trigger: 'blur'
            }
          ]
        }
    ...
    getSliderMax() {
      ...
      for (let i = 1; i <= this.sliderMax; i++) {
        sliderMarks[i] =
          i == 1
            ? '1' + '(' + this.$t('userManage.lowest') + ')'
            : i == this.sliderMax
            ? this.sliderMax + '(' + this.$t('userManage.highest') + ')'
            : String(i)
      }
    ...
    submitForm() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          if (!this.$parent.operation) {
            Object.assign(data, { userId: form.userId })
            this.$api.modifyUserInfo(data).then(res => {
