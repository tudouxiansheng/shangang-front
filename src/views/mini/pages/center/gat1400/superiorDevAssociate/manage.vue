<template>
  <div
    id="superiorDevAssociate"
    v-if="isRouterAlive"
    ref="superiorDevAssociate"
      ...
      <el-col :span="20" class="el-row20">
        ...
        <div class="search-form">
          <el-form
            ...
            :label-width="locale == 'en' ? '160px' : '84px'"
            ref="form"
            :model="searchForm"
            label-position="left"
          >
            <div class="flexstart">
              ...
              <el-form-item :label="$t('deviceManagement.deviceStatus') + ':'">
                <el-select v-model="searchForm.isOnline" clearable>
                  <el-option
...
export default {
  ...
  methods: {
    ...
    handleNodeClick(data, node, self) {
      //点击树联动面包屑
      this.$refs.breadcrumb.treeLinkBread_lane(data, node, self)
    ...
    getViewPlatList() {
      ...
      this.$api.getRelateApeList(data).then(res => {
      ...
      this.typeList = [
        {
          ...
          value: this.$t('deviceManagement.acquisitionEquipment')
        }
      ]
    ...
    handleFind() {
      if (!this.choosedPid) {
        this.$message({
          type: 'warning',
          message: this.$t('cascadeDevice.selectPlatform')
        })
        return
      }
      this.pageNum = 1
    ...
    handleAss() {
      if (!this.choosedPid) {
        this.$message({
          type: 'warning',
          message: this.$t('cascadeDevice.selectBeforeAssociation')
        })
        return
      }
    ...
    async batchOperate() {
      ...
      this.what = this.$t('deviceManagement.device')
      this.operate = this.$t('public.batchAssociate')
      this.batchApi = 'relate'
      this.whatId = 'id'
      this.whatName = 'name'
      this.multipleSelection = this.$refs.devAss.multipleSelection
      this.multipleSelectionMany = []
