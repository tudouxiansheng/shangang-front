        ...
        <el-breadcrumb-item v-for="(item, index) in breadcrumbList" :key="index"
            @click="
                item.label == '>>' || item.label == '<<'
                    ? clickBreadPoint(item)
                    : clickBread(item)
                " @mouseover="item.label == '· · ·' ? overBread(index) : ''"
            @mouseleave="
                item.label == '>>' || item.label == '<<' ? outBread(index) : ''
                ">
            <template #default>
                <span
                    v-if="item.label && (item.label.length < 10 || item.label.length == 10)"
                    :style="item.label == '· · ·' ? 'fontSize:12px' : ''">
                    {{ item.label }}
                </span>
                <el-tooltip effect="dark" :content="item.label" placement="top"
                    v-if="item.label && item.label.length > 10">
                    <span>{{
...
export default {
  ...
  methods: {
    ...
    treeLinkBread(data, node) {
      this.breadcrumbList = []
      this.getAllParent(node)
      this.getBreadCopy()
    },
    treeLinkBread_lane(data, node) {
      this.breadcrumbList = []
    ...
    getBreadCopy() {
      ...
      if (breadcrumbList_noPoint.length > 9) {
       ...
       if(!this.breadcrumbList[0] ||!this.breadcrumbList[0]?.label){
