<template>
  <div class="cover-config config-form">
    ...
    >
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item :label="$t('mainDevConfiguration.overTime')" prop="autoClearAlarmTime">
            <el-input
              :placeholder="$t('mainDevConfiguration.inputOverTime')"
        ...
        <el-col :span="8">
          <el-form-item :label="$t('mainDevConfiguration.videoSensitivity')" prop="sensitivity">
            <el-input
              :placeholder="$t('mainDevConfiguration.inputVideoSensitivity')"
...
export default {
        ...
        autoClearAlarmTime: [
          {
            ...
            message: this.$t('mainDevConfiguration.autoClearAlarmTimeProp'),
            trigger: 'change'
          }
        ],
        sensitivity: [
  ...
  methods: {
    async getCameraInfo() {
      let res = await this.$api.getDevConfig({ deviceCode: this.selectedCameraId, configType: 10 })
      if (res.resultCode === 0) {
    ...
    submitForm() {
      ...
      this.$refs['form'].validate(async valid => {
        if (valid) {
          const { enableVideoHide, guardTimeList, sensitivity, autoClearAlarmTime } = this.form
          ...
          let res = await this.$api.setDevConfig(videoHideAlarm)
          if (res.resultCode === 0) {
            this.$message({
