        ...
        <div class="leftBox">
          <div class="imgBox">
            <el-image
              style="height: 100%"
              :src="form.captureUrl"
              :preview-src-list="[form.captureUrl]"
            ></el-image>
          </div>
        </div>
        <div class="rightBox">
          <div class="rightImgItem">
            <el-image
              ...
              :src="form.detectUrl"
              :preview-src-list="[form.detectUrl]"
            ></el-image>
          </div>
          <ul style="max-height: 300px; overflow-y: auto">
            ...
            <li class="content-item">
              ...
              <span class="content-item-right">{{ form.cameraName }}</span>
            </li>
...
export default {
  ...
  methods: {
    ...
    // 实时浏览
    goToRealtime() {
      const { deviceId, cameraName } = this.form
      goToRealtimePlay(deviceId, cameraName)
    },
    // 录像回放
    goToReplay() {
      const { deviceId, cameraName, captureTime } = this.form
      let param = {
        cameraId: deviceId,
        cameraName,
        alarmTime: captureTime
      }
      window.webkitProc('alarm-playback', JSON.stringify(param))
    },
    // 添加至布控库
    goToRepository() {
      this.imageForRepository = this.form.detectUrl
    ...
    async goToPersonApplication(hitType) {
      if (hitType === '2' && this.locale == 'en') {
        ...
        const { detectUrl } = this.form
        try {
          this.uploadLoading = true
          const blob = await getBlobByGET(detectUrl)
          const file = new File(
            [blob],
    ...
    handleCommand(cmditem) {
      switch (cmditem) {
        case 'a':
          this.downloadImage(this.form.captureUrl)
          break
        case 'b':
          this.downloadImage(this.form.detectUrl)
          break
      }
    },
