<template>
  ...
  <div class="eventDetail">
    ...
    <div class="formemit">
        ...
        <el-row v-show="form.processMode == 1">
          <el-col :span="12">
            <el-form-item label="工单名称:" prop="workSheetName">
              <el-input v-model="form.workSheetName" :disabled="modifyEdit"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
      ...
      <el-form :model="formData" ref="elform2" class="elform" v-show="form.processMode == 1">
        <div v-for="(item, index) in formData.handlerRelatedList" :key="item.handlerId">
          <el-form-item
            label="处理人"
          ...
          >
            <div class="mywrapper">
              ...
              <img src="../../../../../assets/common/add.png" @click="addUser" class="myimg" v-if="item.add" />
              <img
...
export default {
  data() {
    return {
      ...
      eventId: this.$store.state.alarmEventId,
      rules: {
        workSheetName: [{ required: true, trigger: 'change', message: '工单名称必填' }],
        workSheetType: [{ required: true, trigger: 'change', message: '工单类型必选' }],
        workSheetLevel: [
          {
            required: true,
            trigger: 'change',
    ...
    this.formData.handlerRelatedList = [
      {
        ...
        description: '',
        add: true,
        // fujian: this.$parent.currentRow.alarmSnapUrl||'',
      },
    ]
  },
  ...
  methods: {
    ...
    submiteventDetail() {
      ...
      if (Number(this.form.processMode) == 1) {
        ...
        this.form.eventImgUrl = this.$store.state.alarmItem.alarmSnapUrl
        this.form.startTime !== ''
          ? (this.form.startTime = getFormatDate(this.form.startTime))
          : (this.form.startTime = '')
        this.form.endTime !== '' ? (this.form.endTime = getFormatDate(this.form.endTime)) : (this.form.endTime = '')
        this.form.site = this.$store.state.alarmItem.alarmLocate
        this.form.eventTime = this.$store.state.alarmItem.alarmTime
      ...
      if (Number(this.form.processMode) == 0) {
        ...
        this.form.eventId = this.$store.state.alarmEventId
