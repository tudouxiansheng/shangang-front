  ...
  <div id="alarm-channel">
        ...
        >
          <el-row v-for="(item, index) in AlarmOutConfig.AlarmOutList" :key="index">
            ...
            <el-col
              :span="locale == 'en' ? 7 : 8"
              class="pr50"
              v-if="drive != DRIVE_TYPE.ONVIF || platType == PLATFORM_TYPE.VPAAS"
            >
              <el-form-item
                :label="$t('frontParameterConfiguration.alarmConfig.autoClearAlarmTime') + '：'"
            ...
            >
              <el-form-item
                :label="$t('frontParameterConfiguration.alarmConfig.alarmOutMode') + '：'"
                :prop="`AlarmOutList[${index}].alarmOutMode`"
              >
                <el-select
                  v-model="item.alarmOutMode"
                  :placeholder="$t('frontParameterConfiguration.alarmConfig.selectAlarmOutMode')"
                >
                  <el-option
                    v-for="item in alarmOutModeList"
                    :key="item.value"
                    :label="item.name"
                    :value="item.value"
                  />
    ...
    <div class="footer">
      ...
      <el-button
        ...
        :loading="maskloading"
        :disabled="
          (configType == 14 && !AlarmInConfig.AlarmInList.length) ||
          (configType == 15 && !AlarmOutConfig.AlarmOutList.length)
        "
        ...
        AlarmOutList: [
          {
            ...
            alarmOutChanIndex: 1,
            holdTime: 0,
            alarmOutMode: undefined
          }
        ]
      },
      alarmOutModeList: [
        {
          name: this.$t('frontParameterConfiguration.alarmConfig.alarmOutMode0'),
          value: 0
        },
        {
          name: this.$t('frontParameterConfiguration.alarmConfig.alarmOutMode1'),
          value: 1
        }
      ],
      AlarmInList: {
