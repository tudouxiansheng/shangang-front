        ...
        <div class="bg">
          ...
          <div class="tables" style="width: 100%">
            <CustomTable
              ...
              :total="total"
            >
              <el-table-column prop="deviceName" :label="$t('application.deviceName')"> </el-table-column>
              ...
              <el-table-column prop="snapPhoto" :label="$t('passageway.photo')">
                <template slot-scope="scope">
                  <img v-viewer :src="scope.row.snapPhoto" class="snap-photo" v-if="scope.row.snapPhoto" />
                  <img src="@/assets/faceSearch/nan.png" class="snap-photo" v-else />
                </template>
              </el-table-column>
              <el-table-column prop="eventTime" :label="$t('passageway.alarmTime')" min-width="80"> </el-table-column>
...
export default {
  ...
  methods: {
    // 设备名称
    deviceNameLists() {
      this.$fetch('/uas/v1/api/scs/person/vehicle/devName/list', { type: 1 }).then((res) => {
        if (res.resultCode === 0) {
    ...
    // 位置
    installLocateLists() {
      this.$fetch('/uas/v1/api/scs/person/vehicle/installLocation/list', { type: 1 }).then((res) => {
        if (res.resultCode === 0) {
