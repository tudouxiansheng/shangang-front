  ...
  function C(e, a, r) {
    var t = a.length,
      i =
        ...
        1 +
        (function (e) {
          for (var a = e.length, r = 0, t = 0; t < a; ++t)
            r += n.Color.packedLength + 3 + e[t].batchIds.length;
          return r;
        })(r),
    ...
    (s[f++] = e), (s[f++] = t);
    for (var d = 0; d < t; ++d)
      o.OrientedBoundingBox.pack(a[d], s, f), (f += o.OrientedBoundingBox.packedLength);
    var c = r.length;
    s[f++] = c;
    for (var u = 0; u < c; ++u) {
      var h = r[u];
      n.Color.pack(h.color, s, f),
        (f += n.Color.packedLength),
        (s[f++] = h.offset),
        (s[f++] = h.count);
      var l = h.batchIds,
        g = l.length;
  ...
  return s(function (s, f) {
    ...
    for (h in ie)
      ie.hasOwnProperty(h) &&
        (($ = ie[h]),
        ce.push({
          color: n.Color.fromRgba(parseInt(h)),
          offset: $.indexOffset,
          count: $.indexLength,
          batchIds: $.batchIds,
        }));
    for (c = 0; c < q; ++c) {
      ...
      ((P = T).west = Ie),
        ...
        (P.south = ve),
        (P.north = Ce),
        (L[c] = o.OrientedBoundingBox.fromRectangle(P, F, S, U));
      var Oe = $.indexOffset,
        Ue = K[c],
        Pe = p[c];
    ...
    for (var _e = ce.length, Ge = 0; Ge < _e; ++Ge) {
      for (var Ye = ce[Ge].batchIds, Ve = 0, He = Ye.length, We = 0; We < He; ++We)
        Ve += re[Ye[We]];
      ce[Ge].count = Ve;
    }
    var ze = C(
      2 === te.BYTES_PER_ELEMENT ? t.IndexDatatype.UNSIGNED_SHORT : t.IndexDatatype.UNSIGNED_INT,
      L,
      ce
    );
    return (
      f.push(ee.buffer, te.buffer, ne.buffer, re.buffer, ae.buffer, ze.buffer),
      {
        positions: ee.buffer,
        indices: te.buffer,
        indexOffsets: ne.buffer,
        indexCounts: re.buffer,
        batchIds: ae.buffer,
        packedBuffer: ze.buffer,
      }
    );
  });
});
